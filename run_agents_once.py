...
from futureshow.utils.general_tools import write_config_value
...
async def _run_agents_once(agents_info: List[Dict[str, Any]]) -> None:
    for info in agents_info:
        ...
        try:
            ...
            print(f"❌ 模型 {model_name}（{signature}）执行失败: {exc}")
        else:
            print(f"✅ 模型 {model_name}（{signature}）执行完成")


async def main(config_path: Optional[str], limit: int) -> None:
    ...
    await _run_agents_once(agents_info)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="顺序运行 Polymarket Agents 的一次性交易回合")
...
if __name__ == "__main__":
    args = _parse_args()
    try:
        asyncio.run(main(args.config, args.limit))
    except KeyboardInterrupt:
        print("⏹️ 手动停止 Agent 执行")
